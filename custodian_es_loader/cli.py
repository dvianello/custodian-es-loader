...
@click.command()
@click.option("-r", "--resources", type=click.Path(), help="Path to resources.json file")
@click.option("-m", "--metadata", type=click.Path(), help="Path to metadata.json file")
@click.option("-f", "--folder", type=click.Path(), help="Folder to recursively load policies results from")
    ...
    if folder:
        ...
        for metadata_file in metadata_files:
            ...
            if not resources_file.exists() or not resources_file.is_file():
                print("Unable to find file {}. Skipping.".format(resources_file))
                continue
